...
description: "[CCGS converted] Break a single epic into implementable story files. Reads the epic, its GDD, governing ADRs, and control manifest. Each story embeds its GDD requirement TR-ID, ADR guidance, acceptance criteria, story type, and test evidence path. Run after /create-epics for each epic."
---

> Converted from Claude Code Game Studios slash command `/create-stories` for Codex use. Claude-specific tool/model/hook fields were omitted from frontmatter.
