...
description: "[CCGS converted] End-of-story completion review. Reads the story file, verifies each acceptance criterion against the implementation, checks for GDD/ADR deviations, prompts code review, updates story status to Complete, and surfaces the next ready story from the sprint."
---

> Converted from Claude Code Game Studios slash command `/story-done` for Codex use. Claude-specific tool/model/hook fields were omitted from frontmatter.
