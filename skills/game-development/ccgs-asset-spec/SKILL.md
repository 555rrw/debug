...
description: "[CCGS converted] Generate per-asset visual specifications and AI generation prompts from GDDs, level docs, or character profiles. Produces structured spec files and updates the master asset manifest. Run after art bible and GDD/level design are approved, before production begins."
---

> Converted from Claude Code Game Studios slash command `/asset-spec` for Codex use. Claude-specific tool/model/hook fields were omitted from frontmatter.
