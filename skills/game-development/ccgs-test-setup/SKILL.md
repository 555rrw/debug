...
description: "[CCGS converted] Scaffold the test framework and CI/CD pipeline for the project's engine. Creates the tests/ directory structure, engine-specific test runner configuration, and GitHub Actions workflow. Run once during Technical Setup phase before the first sprint begins."
---

> Converted from Claude Code Game Studios slash command `/test-setup` for Codex use. Claude-specific tool/model/hook fields were omitted from frontmatter.
