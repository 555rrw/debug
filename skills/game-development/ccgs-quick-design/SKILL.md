...
description: "[CCGS converted] Lightweight design spec for small changes — tuning adjustments, minor mechanics, balance tweaks. Skips full GDD authoring when a system GDD already exists or the change is too small to warrant one. Produces a Quick Design Spec that embeds directly into story files."
---

> Converted from Claude Code Game Studios slash command `/quick-design` for Codex use. Claude-specific tool/model/hook fields were omitted from frontmatter.
