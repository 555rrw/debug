...
description: "[CCGS converted] Analyze a feature or sprint for scope creep by comparing current scope against the original plan. Flags additions, quantifies bloat, and recommends cuts. Use when user says 'any scope creep', 'scope review', 'are we staying in scope'."
---

> Converted from Claude Code Game Studios slash command `/scope-check` for Codex use. Claude-specific tool/model/hook fields were omitted from frontmatter.
