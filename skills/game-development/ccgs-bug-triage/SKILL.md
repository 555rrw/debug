...
description: "[CCGS converted] Read all open bugs in production/qa/bugs/, re-evaluate priority vs. severity, assign to sprints, surface systemic trends, and produce a triage report. Run at sprint start or when the bug count grows enough to need re-prioritization."
---

> Converted from Claude Code Game Studios slash command `/bug-triage` for Codex use. Claude-specific tool/model/hook fields were omitted from frontmatter.
