...
description: "[CCGS converted] Generate engine-specific test helper libraries for the project's test suite. Reads existing test patterns and produces tests/helpers/ with assertion utilities, factory functions, and mock objects tailored to the project's systems. Reduces boilerplate in new test files."
---

> Converted from Claude Code Game Studios slash command `/test-helpers` for Codex use. Claude-specific tool/model/hook fields were omitted from frontmatter.
