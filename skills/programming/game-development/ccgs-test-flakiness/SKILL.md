...
description: "[CCGS converted] Detect non-deterministic (flaky) tests by reading CI run logs or test result history. Aggregates pass rates per test, identifies intermittent failures, recommends quarantine or fix, and maintains a flaky test registry. Best run during Polish phase or after multiple CI runs."
---

> Converted from Claude Code Game Studios slash command `/test-flakiness` for Codex use. Claude-specific tool/model/hook fields were omitted from frontmatter.
