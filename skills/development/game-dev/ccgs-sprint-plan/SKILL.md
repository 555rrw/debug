...
description: "[CCGS converted] Generates a new sprint plan or updates an existing one based on the current milestone, completed work, and available capacity. Pulls context from production documents and design backlogs."
---

> Converted from Claude Code Game Studios slash command `/sprint-plan` for Codex use. Claude-specific tool/model/hook fields were omitted from frontmatter.
