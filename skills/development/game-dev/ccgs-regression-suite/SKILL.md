...
description: "[CCGS converted] Map test coverage to GDD critical paths, identify fixed bugs without regression tests, flag coverage drift from new features, and maintain tests/regression-suite.md. Run after implementing a bug fix or before a release gate."
---

> Converted from Claude Code Game Studios slash command `/regression-suite` for Codex use. Claude-specific tool/model/hook fields were omitted from frontmatter.
